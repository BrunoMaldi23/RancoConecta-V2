...
New Supabase Auth users create `public.profiles` automatically through the audited `handle_new_auth_user()` trigger. Every user starts as `customer` with `account_status = active`; metadata is used only for `full_name`.
